...
export const isTime = input => {
  ...
  return input.length !== 5 || input.indexOf(':') !== 2 ? false : split.length === 2 && isBetween(split[0], 0, 23) && isBetween(split[1], 0, 59)
}
...
export const isCreditCard = input => {
  ...
  return splitInput.length === 1 ? isAlphanumeric(input) && input.length === 16 : splitInput.every((curr, index, arr) => arr.length === 4 && curr.length === 4 && isAlphanumeric(curr))
}
...
export const isHex = input => {
  const splitInput = input.split('')
  return splitInput.shift() !== '#' ? false : splitInput.every(function (curr, index, arr) {
    return arr.length === 3 || arr.length === 6 && isHexChar(curr)
  })
}

export const isRGB = input => input
  .slice(0, 4) !== 'rgb(' || input.slice(input.length - 1) !== ')' ? false : input.slice(4, input.length - 2).split(',')
...
export const isHSL = input => {
  ...
  return values.length === 3 && isBetween(values.shift(), 0, 360) ? isBetween(values.shift(), 0, 1) && isBetween(values.shift(), 0, 1) : false
}

export const isColor = input => isHex(input) || isRGB(input) || isHSL(input)

export const isTrimmed = input => input[0] === ' ' || input[input.length - 1] === ' ' ? false : input.split('').every(function (curr, index, arr) {
